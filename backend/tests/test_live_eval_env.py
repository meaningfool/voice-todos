...
def test_hosted_dataset_locking_validation_warning_requires_tracked_run_prereqs(
    monkeypatch, tmp_path
):
    env_file = tmp_path / ".env"
    env_file.write_text(
...
def test_stale_benchmark_detection_validation_warning_requires_dataset_token(
    monkeypatch, tmp_path
):
    env_file = tmp_path / ".env"
    env_file.write_text("")
